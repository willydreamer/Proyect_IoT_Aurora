...
public class ListaEquiposAdapterSupervisor2 extends RecyclerView.Adapter<ListaEquiposAdapterSupervisor2.EquipoAdminViewHolder>{
    ...
    public ListaEquiposAdapterSupervisor2.EquipoAdminViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        db = FirebaseFirestore.getInstance();
        View view = LayoutInflater.from(context).inflate(R.layout.item_lista_equipos, parent, false);
        return new ListaEquiposAdapterSupervisor2.EquipoAdminViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ListaEquiposAdapterSupervisor2.EquipoAdminViewHolder holder, int position) {
        db = FirebaseFirestore.getInstance();

        EquipoAdmin e = listaEquipos.get(position);
        holder.equipo = e;

        TextView nombreEquipo = holder.itemView.findViewById(R.id.textNumeroSerie2);
        nombreEquipo.setText(e.getMarca()+" - "+e.getModelo());

        TextView numeroDeSerie = holder.itemView.findViewById(R.id.textTituloEquipo2);
